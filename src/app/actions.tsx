...
export async function exportCourseParticipantsCsv(courseId: string) {
  ...
  try {
    ...
    console.error("createTrainerSessionCheckout Stripe error:", err)
    return { error: "Zahlungslink konnte nicht erstellt werden." }
  }
}
...
export async function createCourseCheckoutSession(
  ...
  let coursePaymentIntentData: any = buildClubPaymentIntentData(club)
    ...
    try {
      ...
      console.error("createCourseCheckoutSession Stripe error:", err)
      return { error: "Zahlungslink konnte nicht erstellt werden." }
    }
  }
  ...
  try {
    ...
    console.error("enrollCourse Stripe error:", err)
    return { error: "Zahlungslink konnte nicht erstellt werden." }
  }
}
...
export async function createMembershipCheckout(
  ...
  try {
    ...
    console.error("createMembershipCheckout Stripe error:", err)
    return { error: "Zahlungslink konnte nicht erstellt werden: " + (err?.message || "Unbekannter Fehler") }
  }
}
...
export async function cancelBooking(bookingId: string) {
  ...
  if (booking.payment_status === 'paid_stripe' && booking.price_paid > 0) {
    ...
    message = `Storniert. Dein Gutschein-Code über ${booking.price_paid}â‚¬ lautet: ${code}`
