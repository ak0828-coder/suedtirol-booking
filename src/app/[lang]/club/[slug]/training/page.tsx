  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7]">
      <div className="mx-auto max-w-6xl px-6 py-10 space-y-10">
        ...
        >
          <div>
            <h1 className="text-3xl font-semibold text-slate-900">{t("training.hero.title", "Training")}</h1>
            <p className="text-slate-500">{t("training.hero.subtitle", "Trainerstunden buchen und Kurse auswählen.")}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Link href={`/${lang}/club/${slug}`}>
              <Button variant="outline" className="rounded-full">{t("training.hero.back", "Zur Startseite")}</Button>
            </Link>
            <Suspense fallback={null}>
              <TourLauncher tour="training" storageKey="tour_training_seen" label={t("training.hero.guide", "Guide")} autoStart />
            </Suspense>
          </div>
        </div>
        ...
        <section id="tour-training-trainers" className="space-y-4">
          <div className="flex items-end justify-between gap-3">
            <h2 className="text-xl font-semibold text-slate-900">{t("training.trainers.title", "Trainer")}</h2>
            {!user ? (
              <div className="text-xs text-slate-500">{t("training.trainers.login_required", "Login nötig, um zu buchen.")}</div>
            ) : null}
          </div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            {(trainers || []).length === 0 ? (
              <div className="text-sm text-slate-500">{t("training.trainers.empty", "Aktuell sind keine Trainer verfügbar.")}</div>
            ) : null}
          </div>
        </section>

        <section id="tour-training-courses" className="space-y-4">
          <h2 className="text-xl font-semibold text-slate-900">{t("training.courses.title", "Kurse & Camps")}</h2>
          {(courses || []).length === 0 ? (
            <div className="text-sm text-slate-500">{t("training.courses.empty", "Aktuell sind keine Kurse veröffentlicht.")}</div>
          ) : (
            <CourseGrid
              clubSlug={slug}
