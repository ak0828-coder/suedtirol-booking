          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <Link href={`/${lang}/club/${slug}/training`}>
              <Button id="tour-booking-training" variant="outline" className="gap-2 rounded-full btn-press">
                <Sparkles className="w-4 h-4" /> {t("club.nav.training", "Training")}
              </Button>
            </Link>
            ...
            {user ? (
              <Link href={`/${lang}/club/${slug}/dashboard`}>
                <Button variant="outline" className="gap-2 rounded-full btn-press">
                  <User className="w-4 h-4" /> {t("club.nav.my_area", "Mein Bereich")}
                </Button>
              </Link>
            ) : (
      ...
      <div className="mx-auto max-w-5xl app-pad mt-8 sm:mt-10">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start gap-4">
                          ...
                          <div className="text-right">
                            ...
                            <span className="text-xs text-slate-400">/ {duration} {t("club.courts.minutes", "Min")}</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        {courtCount === 0 && (
          <div className="mt-8 rounded-2xl border border-dashed border-slate-200/70 bg-white/70 p-8 text-center text-sm text-slate-500 dark:border-slate-800/70 dark:bg-slate-900/70 dark:text-slate-400">
            {t("club.courts.empty", "Noch keine Plätze verfügbar.")}
          </div>
        )}
        ...
        <footer className="mt-16 border-t border-slate-200/60 pt-6 text-sm text-slate-500 dark:border-slate-800/60 dark:text-slate-400">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            ...
            <Link href={`/${lang}/club/${slug}/impressum`} className="text-slate-700 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white">
