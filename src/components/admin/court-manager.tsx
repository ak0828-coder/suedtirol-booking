...
export function CourtManager({
  ...
  async function handleDelete(id: string) {
    if (!confirm(t("admin_courts.confirm_delete", "Platz wirklich löschen?"))) return
    ...
    <Card className="rounded-2xl border border-slate-200/60 bg-white/80 shadow-sm">
      ...
      <CardContent>
        <div className="space-y-4 mb-6">
          ...
          {courts.length === 0 && <p className="text-slate-500 italic">{t("admin_courts.empty", "Keine Plätze angelegt.")}</p>}
        </div>
