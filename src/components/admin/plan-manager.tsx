    ...
    <Card className="h-full rounded-2xl border border-slate-200/60 bg-white/80 shadow-sm">
      ...
      <CardContent className="space-y-6">
            ...
            <div key={p.id} className="p-4 border border-slate-200/60 rounded-xl bg-white hover:shadow-sm transition-shadow space-y-3">
              ...
              {editingId === p.id && (
                <div className="grid gap-3">
                  ...
                  />
                  <Input
                    placeholder="Button-Text (z.B. Jetzt Mitglied werden)"
        ...
        <div className="grid gap-3">
          ...
          <Input type="number" placeholder="Preis in €" value={price} onChange={e => setPrice(e.target.value)} />
          <Textarea placeholder="Kurzbeschreibung" value={description} onChange={e => setDescription(e.target.value)} rows={3} />
          <Input placeholder="Button-Text (z.B. Jetzt beitreten)" value={ctaLabel} onChange={e => setCtaLabel(e.target.value)} />
          <Textarea placeholder="Features (eine pro Zeile)" value={features} onChange={e => setFeatures(e.target.value)} rows={3} />
          <Button onClick={handleAdd} disabled={isLoading} className="bg-slate-900 text-white rounded-full">
            {isLoading ? <Loader2 className="animate-spin" /> : "Plan hinzufügen"}
          </Button>
        </div>
      </CardContent>
    </Card>
